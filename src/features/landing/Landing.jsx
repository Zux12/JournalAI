...
<button
  className="btn"
  onClick={()=>{
    const ok = confirm('Start a new project?\nThis will clear the current project in this browser.');
    if (!ok) return;
    try {
      // Optional: clear saved preferences so toggles return to defaults
      localStorage.removeItem('journalai.prefs');
    } catch {}
    resetProject();
    nav('/metadata');
  }}
